...
public class ThreadAlarmListener {
	...
	public void handleThreadMentionedEvent(ThreadMentionedEvent event) {
		log.info(
			"스레드 멘션 이벤트 수신: 스레드 Id={}, 멘션된 프로젝트 멤버 Id={}",
			event.getThreadMessage().getId(),
			event.getProjectMember().getId()
		);
		Long memberId = projectMemberService.getMemberIdByProjectMemberId(event.getProjectMember().getId());
